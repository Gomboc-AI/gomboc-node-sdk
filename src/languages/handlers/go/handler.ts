...
export class GoLanguageHandler extends BaseLanguageHandler {
  displayName = 'Go';
  codeResourceType = 'go';
  extensions = ['.go'];
  ...
  /**
   * Parses Go type/function declarations into brace-delimited ranges.
   */
  private parseBlocks(content: string): BlockRange[] {
    ...
    const functionPattern =
      /^\s*func\s*(?:\(\s*[^)]*\)\s*)?([A-Za-z_][A-Za-z0-9_]*)\s*\([^)]*\)[^{]*\{/;
    const controlKeywords = new Set(['if', 'for', 'switch', 'select']);
