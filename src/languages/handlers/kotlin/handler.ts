...
export class KotlinLanguageHandler extends BaseLanguageHandler {
  displayName = 'Kotlin';
  codeResourceType = 'kotlin';
  extensions = ['.kt'];
  ...
  /**
   * Parses Kotlin type/function declarations into brace-delimited ranges.
   */
  private parseBlocks(content: string): BlockRange[] {
    const lines = content.split('\n');
    const blocks: BlockRange[] = [];
    const typePattern =
      /^\s*(?:public|private|protected|internal|open|final|abstract|sealed|data|enum|annotation|inner|\s)*(class|interface|object)\s+([A-Za-z_][A-Za-z0-9_]*)\b[^{;]*\{/;
    const functionPattern =
      /^\s*(?:public|private|protected|internal|open|final|abstract|override|suspend|inline|tailrec|operator|infix|external|const|lateinit|\s)*fun\s+(?:<[^>]+>\s*)?([A-Za-z_][A-Za-z0-9_]*)\s*\([^;{}]*\)\s*(?::\s*[^{=]+)?\{/;
